...
export const ServiceCard1 = () => {
  ...
  return (
    <div className="group h-[420px] w-full [perspective:1000px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      {/* ZMIANA 1: Zamiast lg:group-hover używamy [@media(hover:hover)]:group-hover 
        To omija problem "zablokowanego hovera" na tabletach
      */}
      <div className={`relative h-full w-full rounded-2xl transition-all duration-700 [transform-style:preserve-3d] will-change-transform ${isFlipped ? '[transform:rotateY(180deg)]' : '[@media(hover:hover)]:group-hover:[transform:rotateY(180deg)]'} shadow-lg hover:shadow-2xl`}>
        ...
        {/* PRZÓD KARTY - dodano [-webkit-backface-visibility:hidden] dla starych iPhone'ów */}
        <div className="absolute inset-0 h-full w-full rounded-2xl overflow-hidden [backface-visibility:hidden] [-webkit-backface-visibility:hidden] border-2 border-[#3f8ace] shadow-lg">
          ...
          {/* ZMIANA 2: TARCZA OCHRONNA (Rozwiązuje problem widocznych napisów)
            Ten div otacza wszystkie napisy na przodzie i znika (opacity-0) w momencie obrotu!
          */}
          <div className={`absolute inset-0 transition-opacity duration-500 ${isFlipped ? 'opacity-0' : 'opacity-100 [@media(hover:hover)]:group-hover:opacity-0'}`}>
            ...
            <div className="absolute inset-0 p-8 flex flex-col items-center justify-center text-center z-10">
              ...
              <div className="w-20 h-20 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center mb-6 text-white border border-white/20 shadow-xl">
                  <ShieldCheck size={40} />
              </div>
        ...
        {/* TYŁ KARTY */}
        <div className="absolute inset-0 h-full w-full rounded-2xl bg-[#3f8ace] p-8 text-white [transform:rotateY(180deg)] [backface-visibility:hidden] [-webkit-backface-visibility:hidden] flex flex-col justify-center">
...
export const ServiceCard2 = () => {
  ...
  return (
    <div className="group h-[420px] w-full [perspective:1000px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      {/* Kontener główny z poprawionym hoverem dla tabletów */}
      <div className={`relative h-full w-full rounded-2xl transition-all duration-700 [transform-style:preserve-3d] will-change-transform ${isFlipped ? '[transform:rotateY(180deg)]' : '[@media(hover:hover)]:group-hover:[transform:rotateY(180deg)]'} shadow-md hover:shadow-xl`}>
        ...
        <div className="absolute inset-0 h-full w-full rounded-2xl overflow-hidden [backface-visibility:hidden] [-webkit-backface-visibility:hidden] shadow-md border border-slate-700">
          ...
          {/* Warstwa frontowa, która znika całkowicie po obróceniu (rozwiązuje problem prześwitywania) */}
          <div className={`absolute inset-0 transition-opacity duration-500 ${isFlipped ? 'opacity-0' : 'opacity-100 [@media(hover:hover)]:group-hover:opacity-0'}`}>
            <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/70 to-slate-800/40"></div>
            <div className="absolute inset-0 p-8 flex flex-col items-center justify-center text-center z-10">
              <div className={`w-20 h-20 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center mb-6 text-white border border-white/20 transition-colors duration-500 shadow-xl`}>
                <PaintRoller size={40} />
              </div>
              <h4 className="text-2xl font-extrabold text-white mb-4 drop-shadow-md">Malowanie Hydrodynamiczne</h4>
              <div className="mt-auto flex items-center text-slate-300 font-semibold text-sm drop-shadow-md">
        ...
        {/* TYŁ KARTY */}
        <div className="absolute inset-0 h-full w-full rounded-2xl bg-slate-800 p-8 text-white [transform:rotateY(180deg)] [backface-visibility:hidden] [-webkit-backface-visibility:hidden] flex flex-col justify-center border-b-4 border-[#3f8ace]">
    ...
    <div className="group h-[420px] w-full [perspective:1000px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      ...
      <div className={`relative h-full w-full rounded-2xl transition-all duration-700 [transform-style:preserve-3d] will-change-transform ${isFlipped ? '[transform:rotateY(180deg)]' : '[@media(hover:hover)]:group-hover:[transform:rotateY(180deg)]'} shadow-md hover:shadow-xl`}>
        ...
        <div className="absolute inset-0 h-full w-full rounded-2xl overflow-hidden [backface-visibility:hidden] [-webkit-backface-visibility:hidden] shadow-md border border-slate-700">
          ...
          {/* Warstwa frontowa znika całkowicie przy obrocie, aby nie nakładać się na tekst z tyłu */}
          <div className={`absolute inset-0 transition-opacity duration-500 ${isFlipped ? 'opacity-0' : 'opacity-100 [@media(hover:hover)]:group-hover:opacity-0'}`}>
            <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/70 to-slate-800/40"></div>
            <div className="absolute inset-0 p-8 flex flex-col items-center justify-center text-center z-10">
              <div className="w-20 h-20 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center mb-6 text-white border border-white/20 transition-colors duration-500 shadow-xl">
